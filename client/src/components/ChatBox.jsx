...
const ChatBox = () => {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setPrompt(promptCopy); // revert prompt if failed
      }
    } catch (error) {
  ...
  return (
    <div className="flex-1 flex flex-col justify-between m-5 md:m-10 max-md:mt-14 2xl:pr-40 h-full">
      {/* chat messages */}
      <div ref={containerRef} className="flex-1 mb-1.5 overflow-y-scroll">
        {messages.length === 0 && (
